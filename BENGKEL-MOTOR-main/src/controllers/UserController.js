...
const updateUserProfile = (req, res) => {
    upload(req, res, async (err) => {
        ...
        try {
            // SINKRONKAN: Tangkap 'bio' dan 'status' dari req.body
            const { name, department, bio, status, role } = req.body;
            
            const updateData = { 
                name, 
                department,
                bio: bio || 'Staff profesional Sisfo Bengkel siap melayani Anda.',
                status: status || 'ready'
            };

            // Jika role diedit oleh admin
            ...
            // Cache Buster untuk gambar profil
            const successParam = "?update=success&t=" + Date.now();
            
            // Redirect sinkron berdasarkan role
            const redirectMap = {
            ...
            res.redirect((redirectMap[updatedUser.role] || '/') + successParam);
            
        } catch (error) {
            console.error("Error saat update profil:", error);
            res.status(500).send("Gagal update: " + error.message);
        }
    });
};
// --- Hapus User ---
