...
exports.updateJobStatus = async (req, res) => {
    try {
        ...
        // Sinkronisasi ke Transaksi
        if (job.transactionId) {
            const trxStatus = (status === 'on_progress') ? 'proses' : (status === 'completed' ? 'lunas' : 'proses');
            await Transaction.findByIdAndUpdate(job.transactionId, { status: trxStatus });
        }

        // Sinkronisasi ke User (Mekanik)
        if (status === 'completed') {
