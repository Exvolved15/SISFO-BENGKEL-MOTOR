...
app.use(async (req, res, next) => {
    try {
        ...
        if (token) {
            ...
            res.locals.user = currentUser; // User tersedia di semua EJS
        } else {
            res.locals.user = null; // Pastikan user didefinisikan sebagai null
        }
    } catch (err) {
...
// 1. Rute Publik (Overview & Auth View)
// Overview (Diletakkan di atas agar bisa diakses tanpa login)
app.get('/', protect, (req, res) => {
  const userRole = req.user.role; // Mengambil role dari user yang login
  
  console.log(`User ${req.user.email} login dengan role: ${userRole}`); // Untuk debug di terminal

  if (userRole === 'admin') {
      return res.redirect('/admin/dashboard');
  } else if (userRole === 'kasir') {
      return res.redirect('/kasir/dashboard');
  } else if (userRole === 'mekanik') {
...
app.use((req, res) => {
    ...
    res.render('404', { 
        ...
        activePage: 'error',
        user: res.locals.user // Tetap kirim user jika ada agar navbar tidak error
    });
});
