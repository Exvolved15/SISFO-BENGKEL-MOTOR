...
// 2. DASHBOARD MEKANIK (FIXED)
app.use('/mechanic/dashboard', protect, async (req, res) => {
    try {
        // Ambil pekerjaan yang ditugaskan ke mekanik ini
        const myTasks = await Job.find({ mechanic: req.user._id }).sort({ createdAt: -1 });
        res.render('mechanic/dashboard', { 
            title: 'Mekanik Dashboard', 
            jobs: myTasks, 
            activePage: 'dashboard' 
        });
    } catch (error) {
        res.status(500).send("Gagal memuat data mekanik");
    }
});

// 3. DASHBOARD KASIR (NEW)
app.use('/cashier/dashboard', protect, async (req, res) => {
    try {
        ...
        res.status(500).send("Gagal memuat data kasir");
    }
});
...
// --- D. LOGIKA REDIRECT BERANDA (FIXED) ---
app.get('/', (req, res) => {
    if (req.cookies.token) {
        try {
            ...
            if (decoded.role === 'kasir') return res.redirect('/cashier/dashboard');
