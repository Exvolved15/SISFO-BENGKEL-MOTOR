...
// --- A. RUTE API (WAJIB PALING ATAS) ---
// Letakkan API di sini agar tidak tertelan oleh rute '/' visual
...
// --- B. RUTE TAMPILAN (HTML) ---
app.use('/', authViewRoutes); // Menangani GET /login & /register
app.get('/about', (req, res) => {
    res.render('about', { title: 'Tentang Kami', activePage: 'about' });
});

// Landing Page / Overview (Halaman utama)
app.use('/', viewRoutes); 
...
// --- C. API 404 HANDLER ---
// Pastikan jika rute API salah, tetap kirim JSON, bukan HTML 404
app.all('/api/*', (req, res) => {
    res.status(404).json({
        success: false,
