...
mongoose.plugin((schema) => {
    // Sinkronisasi saat data dibuat baru (save)
    schema.post('save', async function(doc) {
        const collection = this.constructor.modelName.toLowerCase();
        console.log(`[SYNC] Mengirim data ${collection} ke Firebase...`);
        await syncToFirebase(collection, doc);
    });

    // Sinkronisasi saat data diupdate (findOneAndUpdate / updateProfile)
    schema.post('findOneAndUpdate', async function(doc) {
        if (doc) {
            const collection = doc.constructor.modelName.toLowerCase();
            console.log(`[SYNC] Update data ${collection} ke Firebase...`);
            await syncToFirebase(collection, doc);
        }
    });
});
