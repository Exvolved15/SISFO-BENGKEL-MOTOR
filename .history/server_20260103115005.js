...
// ==================================================================
// 2. PASANG MONGOOSE PLUGIN (SINKRONISASI FIREBASE REALTIME)
// ==================================================================
mongoose.plugin((schema) => {
    // Sinkronisasi saat data baru dibuat (Register/Add Part)
    schema.post('save', function(doc) {
        const collection = this.constructor.modelName.toLowerCase();
        console.log(`[SYNC] Menambahkan ke Live Database: ${collection}`);
        syncToFirebase(collection, doc);
    });

    // Sinkronisasi saat data diupdate (Update Profile/Edit Part)
    // Tanpa ini, data di Live Database akan tertinggal/berbeda
    schema.post('findOneAndUpdate', function(doc) {
        if (doc) {
            const collection = doc.constructor.modelName.toLowerCase();
            console.log(`[SYNC] Memperbarui Live Database: ${collection}`);
            syncToFirebase(collection, doc);
        }
    });
});
...
// --- LOGIKA REDIRECT OTOMATIS (FIXED) ---
app.get('/', (req, res) => {
    if (req.cookies.token) {
        try {
            ...
            if (decoded.role === 'kasir') return res.redirect('/kasir/dashboard'); // Sesuai path folder Anda
