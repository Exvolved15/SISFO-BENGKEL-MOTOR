...
// --- PASANG PLUGIN OTOMATIS SEBELUM MODEL DI-IMPORT ---
mongoose.plugin((schema) => {
    // 1. Sinkron saat Data Baru atau Update (.save() / .create())
    schema.post('save', function(doc) {
        const collection = this.constructor.modelName.toLowerCase();
        syncToFirebase(collection, doc);
    });

    // 2. Sinkron saat Update via Query (.findOneAndUpdate)
    ...
    // 3. Hapus di Firebase jika di MongoDB dihapus
...
// Import model-model Anda SETELAH plugin terpasang
