...
app.use(async (req, res, next) => {
  res.locals.activePage = ''; // Default active page
  try {
      const token = req.cookies.jwt;
      if (token) {
          // Verifikasi token dan ambil data user terbaru dari DB
          const decoded = jwt.verify(token, process.env.JWT_SECRET);
          const currentUser = await User.findById(decoded.id);
          
          // Variabel 'user' ini yang dipanggil di navbar.ejs & dashboard.ejs
...
app.get('/', protect, (req, res) => {
  ...
  // Alamat ini HARUS sama dengan yang ada di navbar dan rute jobRoutes
