...
router.get('/transactions/print/:id', protect, async (req, res) => {
    try {
        ...
        // 3. Render file receipt tanpa layout utama agar rapi saat diprint
        res.render('kasir/receipt', {
            layout: false, // Mematikan express-ejs-layouts untuk halaman ini
            title: `Resi: ${header.invoiceNumber}`,
            header: header,
            details: details
        });
    } catch (error) {
        res.status(500).send('Gagal memproses resi: ' + error.message);
    }
});
