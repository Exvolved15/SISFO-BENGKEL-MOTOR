...
router.post('/parts/add', protect, async (req, res) => {
    try {
        // PERBAIKAN: Gunakan Part.create, BUKAN Service.create
        await Part.create({
            name: req.body.name,
            code: req.body.code,
            ...
            isPart: true // Pastikan ditandai sebagai part
        });
        
        console.log("✅ Sukses Menambah Suku Cadang Baru");
        res.redirect('/parts');
    } catch (error) {
        console.error("Gagal tambah part:", error);
        res.render('parts/add', {
            title: 'Tambah Suku Cadang',
            activePage: 'parts',
            user: req.user,
...
router.get('/parts', protect, async (req, res) => {
    try {
        // Ambil SEMUA parts (hapus filter stock > 0 agar barang stok 0 tetap muncul di list admin)
        const parts = await Part.find({}); 
        
        res.render('parts/list', { 
            title: 'Daftar Suku Cadang',
            parts: parts, // Variable yang dikirim bernama 'parts'
            activePage: 'parts',
            user: req.user
        });
    } catch (error) {
