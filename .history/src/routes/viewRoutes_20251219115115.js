...
// src/routes/viewRoutes.js
// === CARI BAGIAN INI (Rute Buat Transaksi) DAN GANTI DENGAN KODE INI ===
router.get('/transactions/add', protect, async (req, res) => {
    try {
        // 1. Ambil Data Parts (Stok > 0)
        const parts = await Part.find({ stock: { $gt: 0 } });
        
        // 2. Ambil Data Service
        const services = await Service.find({});
        
        // 3. AMBIL DATA MEKANIK (INI YANG SEBELUMNYA KURANG)
        const mechanics = await User.find({ role: 'mekanik' });

        // 4. Render Halaman dengan mengirimkan semua variabel
        res.render('transactions/add', { 
            title: 'Buat Transaksi Baru', 
            parts: parts, 
            services: services, 
            mechanics: mechanics, // <--- Variable ini wajib dikirim agar tidak error
            user: req.user
        });
    } catch (error) {
        console.error(error);
