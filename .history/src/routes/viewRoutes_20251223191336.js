...
// FIX: Menambahkan staffList agar Overview Pekerja di Index tampil dinamis
router.get('/', async (req, res) => {
    try {
        if (req.cookies.jwt || req.user) return res.redirect('/dashboard-redirect');
        
        // Mengambil semua user kecuali role pelanggan untuk Overview Pekerja
        ...
        res.status(500).send("Error memuat halaman utama");
    }
});
