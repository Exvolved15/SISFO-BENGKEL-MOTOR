...
router.get('/transactions/add', protect, async (req, res) => {
    try {
        ...
        // Ambil Services
        const services = await Service.find({});
        // Ambil Mekanik (SOLUSI ERROR "mechanics is not defined")
        ...
        res.render('transactions/add', { 
            ...
            parts: parts, 
            services: services, 
            mechanics: mechanics, // <--- Data ini sekarang dikirim!
            user: req.user,
            activePage: 'transactions'
        });
    } catch (error) {
        console.error(error);
        res.status(500).send("Terjadi kesalahan server saat memuat halaman transaksi.");
    }
});
