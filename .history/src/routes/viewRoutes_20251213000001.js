...
router.post('/transactions/add', async (req, res) => {
    ...
    // Perlu tambahkan try-catch di sini untuk memastikan error dari logic ditangani
    try {
        // 1. Parsing itemDetails dari string JSON ke objek/array
        // Ini KUNCI karena data dari form dikirim sebagai string
        req.body.itemDetails = JSON.parse(req.body.itemDetails);
        
        await processTransactionLogic(req.body); // Panggil fungsi Logic
        
        // 2. Jika Logic BERHASIL: Redirect
        return res.redirect('/transactions'); // Gunakan return untuk menghentikan eksekusi selanjutnya

    } catch (error) {
        // 3. Jika Logic GAGAL (Stok, Validasi, dll.): Kirim error response
        console.error("Error Transaksi dari Form:", error.message);
        
        // Pastikan kita tidak mencoba me-render view yang tidak kita miliki (karena belum dibuat)
        // Kirim response status 400 (Bad Request)
