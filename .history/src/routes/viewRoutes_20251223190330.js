...
router.get('/admin/dashboard', protect, restrictTo('admin'), async (req, res) => {
    try {
        ...
        const staff = await User.find({ role: { $ne: 'pelanggan' } }).select('name role profileImage status');
        
        res.render('index', {
            title: 'Overview',
            activePage: 'home',
            staffList: staff, // Kirim data staff ke EJS
...
router.get('/mechanic/dashboard', protect, restrictTo('mekanik', 'admin'), async (req, res) => {
    try {
        // Cari Job berdasarkan field mechanicId
        ...
        res.render('mechanic/dashboard', {
            ...
            user: req.user,
            historyJobs: historyJobs, // Akan di-filter oleh EJS (activeJob, pendingJobs, dsb)
            activePage: 'dashboard'
        });
    } catch (error) {
