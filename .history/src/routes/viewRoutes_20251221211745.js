...
router.get('/customer/dashboard', protect, restrictTo('customer'), async (req, res) => {
    try {
        // 1. Ambil riwayat transaksi milik pelanggan ini
        const transactions = await Transaction.find({ customer: req.user._id }).sort({ createdAt: -1 });

        // 2. Ambil status pekerjaan (Job) aktif untuk kendaraan pelanggan ini
