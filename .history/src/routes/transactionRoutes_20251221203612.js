...
router.route('/:id/receipt')
    ...
    router.patch('/update-status/:id', async (req, res) => {
        try {
            ...
            await Transaction.findByIdAndUpdate(req.params.id, { status: status });
            
            // 2. Jika ada Job terkait, selesaikan juga agar mekanik jadi 'nganggur'
            await Job.findOneAndUpdate({ transactionId: req.params.id }, { status: status });
