...
router.get('/', (req, res) => {
    res.render('index', { 
        ...
        activePage: 'home' // Pastikan ini dikirim agar navbar tidak error
    });
});
...
// Helper rute untuk menentukan dashboard berdasarkan role (Hanya untuk User Login)
...
// Dashboard Mekanik (Hanya melihat Job milik sendiri)
router.get('/mechanic/dashboard', protect, restrictTo('mekanik', 'admin'), async (req, res) => {
    try {
        const activeJob = await Job.findOne({ assignedTo: req.user._id, status: 'on_progress' });
        const historyJobs = await Job.find({ assignedTo: req.user._id, status: 'completed' }).sort({ updatedAt: -1 });
        const pendingJobs = await Job.find({ assignedTo: req.user._id, status: 'pending' }).sort({ createdAt: -1 });
...
// Dashboard Pelanggan (Hanya melihat motor milik sendiri)
router.get('/customer/dashboard', protect, async (req, res) => {
    try {
        // Cari berdasarkan customerId (sinkron dengan logic createTransaction)
