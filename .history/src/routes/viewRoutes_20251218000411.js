...
router.get('/transactions/add', protect, restrictTo('kasir', 'admin'), async (req, res) => {
    try {
        // Ambil data dari database agar bisa dipilih di dropdown
        const allItems = await Service.find();
        const mekaniks = await User.find({ role: 'mekanik' });

        res.render('transactions/add', {
            title: 'Buat Transaksi Baru',
            parts: allItems.filter(item => item.isPart), // Kirim suku cadang
            services: allItems.filter(item => !item.isPart), // Kirim jasa
            mekaniks,
            user: req.user,
            activePage: 'transactions'
        });
...
router.get('/transactions/print/:id', protect, async (req, res) => {
    try {
        const transactionId = req.params.id;
        const header = await Transaction.findById(transactionId);
        // Penting: Gunakan populate jika butuh data relasi di resi
        const details = await TransactionDetail.find({ transactionId: transactionId });

        if (!header) return res.status(404).send('Data transaksi tidak ditemukan.');
        ...
        res.status(500).send('Gagal mencetak resi: ' + error.message);
    }
});
