...
router.get('/transactions/print/:id', protect, async (req, res) => {
    try {
        ...
        // 2. Cari Detail Item (Oli, Jasa, dll) untuk resi
        const details = await TransactionDetail.find({ transactionId: transactionId });

        // 3. Render file receipt.ejs yang sudah Anda miliki
        res.render('transactions/receipt', { 
            title: 'Cetak Struk',
            transaction: transaction,
            details: details,
            layout: false // Penting: Agar CSS navbar tidak merusak tampilan struk
        });
    } catch (error) {
        console.error("Cetak Error:", error);
        res.status(500).send("Gagal memuat resi: " + error.message);
    }
});
