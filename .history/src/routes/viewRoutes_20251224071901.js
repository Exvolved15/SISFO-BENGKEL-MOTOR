...
router.get('/', async (req, res) => {
    try {
        // Jika sudah login melalui middleware di server.js, maka res.locals.user akan ada
        // Logika redirect dipindah ke server.js agar rute ini fokus ke Landing Page
        ...
        res.render('index', { 
            ...
            activePage: 'home',
            staffList: staff, // Data dilempar ke index.ejs
            error: req.query.error || null
        });
    } catch (error) {
        res.render('index', { title: 'Overview', activePage: 'home', staffList: [], error: null });
    }
});
...
router.get('/parts/add', protect, restrictTo('admin'), (req, res) => {
    res.render('parts/add', { title: 'Tambah Barang', user: req.user, activePage: 'parts' });
});
