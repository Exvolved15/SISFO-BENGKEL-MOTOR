...
// FIX: Struktur router.get('/') yang sebelumnya rusak
router.get('/', async (req, res) => {
    try {
        if (req.cookies.jwt || req.user) return res.redirect('/dashboard-redirect');
        
        // Mengambil data staff untuk Overview di Index
...
router.get('/users/edit/:id', protect, async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(req.params.id, { name, bio, department, status });
        res.redirect('/dashboard-redirect'); // Kembali ke dashboard setelah simpan
