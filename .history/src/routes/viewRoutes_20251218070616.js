...
router.get('/kasir/dashboard', protect, restrictTo('kasir', 'admin'), async (req, res) => {
    try {
        // UBAH: Gunakan Job.find() karena Mekanik mengupdate data di model Job
        // Ambil semua job, baik yang masih pending maupun yang sudah completed
        const history = await Job.find()
            .sort({ createdAt: -1 })
            .limit(10); // Ambil 10 agar riwayat lebih terlihat
        
        res.render('kasir/dashboard', {
            title: 'Kasir Dashboard',
            user: req.user,
            transactions: history, // Variabel tetap 'transactions' agar EJS tidak perlu diubah
            activePage: 'dashboard'
        });
    } catch (error) {
...
router.get('/transactions/print/:id', protect, async (req, res) => {
    try {
        const Job = require('../models/Job'); // Import model Job
        const jobData = await Job.findById(req.params.id);
        
        if (!jobData) return res.status(404).send('Data tidak ditemukan');

        res.render('kasir/receipt', {
            layout: false,
            job: jobData // Kirim variabel 'job' sesuai receipt.ejs Anda
        });
    } catch (error) {
        res.status(500).send("Gagal cetak resi");
    }
});
