...
// src/routes/viewRoutes.js
router.get('/transactions/add', protect, restrictTo('kasir', 'admin'), async (req, res) => {
    try {
        // 1. Ambil data suku cadang (isPart: true)
        const parts = await Service.find({ isPart: true });
        
        // 2. Ambil data jasa (isPart: false)
        const services = await Service.find({ isPart: false });
        
        // 3. Ambil data mekanik untuk dropdown penugasan
        const mekaniks = await User.find({ role: 'mekanik' });

        res.render('transactions/add', {
            title: 'Buat Transaksi Baru',
            parts: parts,       // Harus dikirim agar EJS bisa membaca
            services: services,   // Harus dikirim agar EJS bisa membaca
            mekaniks: mekaniks,
            activePage: 'transactions'
        });
    } catch (error) {
        res.status(500).send("Gagal memuat form transaksi: " + error.message);
    }
});
// ===================================
...
router.get('/admin/dashboard', protect, restrictTo('admin'), async (req, res) => {
    try {
        const parts = await Service.find({ isPart: true }).limit(5);
        const transactions = await Job.find().sort({ createdAt: -1 }).limit(5);
...
// Rute cetak resi menggunakan ID dari Job
router.get('/transactions/print/:id', protect, async (req, res) => {
    try {
        ...
        res.render('kasir/receipt', { 
            layout: false, 
            transaction: job // Nama variabel ini harus sama dengan yang dipanggil di EJS
        });
    } catch (error) {
