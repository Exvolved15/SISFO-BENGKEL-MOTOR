...
router.get('/transactions/add', async (req, res) => {
    const allItems = await Service.find(); // Ambil SEMUA data dari database
    const parts = allItems.filter(item => item.isPart);
    const services = allItems.filter(item => !item.isPart);
    const mekaniks = await User.find({ role: 'mekanik' });

    res.render('transactions/add', {
        parts,      // Dikirim ke EJS
        services,   // Dikirim ke EJS
        mekaniks
    });
});
