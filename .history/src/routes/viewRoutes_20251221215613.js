...
router.get('/transactions/add', protect, restrictTo('admin', 'kasir'), async (req, res) => {
    try {
        ...
        // SOLUSI: Ambil daftar pelanggan agar dropdown di EJS tidak error
        const users = await User.find({ role: { $in: ['customer', 'user'] } }).sort({ name: 1 });
        ...
        res.render('transactions/add', { 
            ...
            services: services, 
            mechanics: mechanics, 
            users: users, // Variabel ini sekarang tersedia untuk EJS
            user: req.user,
            activePage: 'transactions'
        });
