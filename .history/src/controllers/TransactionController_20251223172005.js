...
const createTransaction = async (req, res) => {
    try {
        ...
        // C. SINKRON KE TABEL JOB (Agar muncul di Dashboard Mekanik & Pelanggan)
        if (assignedTo && assignedTo !== "null") {
            await Job.create({
                ...
                vehicleLicense: vehicleLicense || "-",
                motorName: motorName || "-",
                mechanicId: assignedTo, // Link ke Dashboard Mekanik (PASTIKAN FIELD INI SAMA)
                description: `Servis: ${jobDescriptionItems.join(', ')}`,
                status: 'pending'
            });

            // Update status Mekanik jadi sibuk di tabel User
