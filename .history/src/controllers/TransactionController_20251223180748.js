...
const createTransaction = async (req, res) => {
    try {
        ...
        // 2. Olah Data Item (Suku Cadang & Jasa)
        const items = typeof itemDetails === 'string' ? JSON.parse(itemDetails) : itemDetails;
        let itemSummary = []; // Array untuk menampung nama-nama item

        if (items && items.length > 0) {
            const detailsData = items.map(item => {
                // Masukkan nama item ke summary untuk deskripsi Job
        ...
        // Hasilnya: "Oli MPX (1x), Service Ringan (1x) | Keterangan: Rem agak blong"
        const fullDescription = itemSummary.join(', ') + (notes ? ` | Ket: ${notes}` : '');

        // 4. SINKRONISASI KE TABEL JOB (Isi Invoice Detail & Notes)
        if (assignedTo && assignedTo !== "null") {
            await Job.create({
                ...
                description: fullDescription, // DATA SUDAH TERISI RINCIAN
                status: 'pending'
            });
