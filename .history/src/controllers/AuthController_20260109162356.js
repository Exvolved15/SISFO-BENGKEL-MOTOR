...
/**
 * @desc    Update Profil User (Mendukung Upload Gambar & Redirect)
 * @route   POST /api/auth/update-profile/:id
 */
const updateProfile = async (req, res) => {
    try {
        const userId = req.params.id;
        const { name, email, role, bio, department, status } = req.body;
        ...
        // 3. Update Foto Profil jika ada file baru yang diunggah
        if (req.file) {
            // Simpan path relatif ke database
            user.profileImage = `/uploads/profile/${req.file.filename}`;
        }

        // 4. Simpan perubahan ke MongoDB
        // Plugin di server.js akan otomatis mensinkronkan ini ke Firebase
        await user.save();

        console.log(`[AUTH] Profil Updated: ${user.email}`);

        // 5. REDIRECT: Alihkan kembali ke dashboard sesuai role yang sedang login
        // Jika Admin mengedit orang lain, tetap kembali ke dashboard Admin
        const currentUserRole = req.user.role; 
        
        if (currentUserRole === 'admin') {
            return res.redirect('/admin/dashboard?status=updated');
        } else if (currentUserRole === 'kasir') {
            return res.redirect('/kasir/dashboard?status=updated');
        } else if (currentUserRole === 'mekanik') {
            return res.redirect('/mechanic/dashboard?status=updated');
        } else {
            return res.redirect('/customer/dashboard?status=updated');
        }

    } catch (error) {
        console.error("UPDATE PROFILE ERROR:", error.message);
        res.status(500).send("Terjadi kesalahan saat memperbarui profil: " + error.message);
    }
};
