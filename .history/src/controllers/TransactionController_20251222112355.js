...
if (mechanicId) {
    try {
        await Job.create({
            invoiceNumber: invoiceNumber, // Sesuai log error sebelumnya
            
            // PERBAIKAN DI SINI:
            // Kirim sebagai objek sesuai permintaan validasi Model Job Anda
            customer: {
                name: finalCustomerName // finalCustomerName diambil dari hasil User.findById(customer)
            },
            
            // Field pendukung lainnya agar sinkron
        ...
        console.error("⚠️ Gagal membuat Job karena validasi:", jobError.message);
        // Jika Job gagal, transaksi tetap aman tapi kita beri tahu di log
    }
}
...
const getTransaction = async (req, res) => {
    try {
        ...
        if (!transaction) return res.status(404).json({ success: false, message: 'Tidak ditemukan' });
