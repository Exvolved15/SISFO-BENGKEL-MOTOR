...
const createTransaction = async (req, res) => {
    try {
        ...
        const newTransaction = await Transaction.create({
            ...
            user: req.user._id // Kasir yang input
        });
        ...
        // 3. SINKRONISASI KE TABEL JOB (Agar muncul di Dashboard Mekanik)
        if (assignedTo) {
            await Job.create({
                invoiceNumber: invoiceNumber,
                customer: { name: finalCustomerName },
                customerId: customer || null, // Digunakan pelanggan untuk filter dashboard
                transactionId: newTransaction._id,
                vehicleLicense: vehicleLicense,
                motorName: motorName || "-",
                assignedTo: assignedTo, // ID Mekanik
                description: `Servis: ${jobDescriptionItems.join(', ')}`,
                status: 'pending'
            });

            // Update status User Mekanik menjadi 'kerja' agar di Dashboard Kasir terlihat sibuk
...
// Fungsi lain (index, getTransaction, printReceipt, trackStatusPublic) tetap...
// ... (salin fungsi lama Anda di sini)

module.exports = {
    index,
    createTransaction, // Pastikan di Routes memanggil fungsi ini
    getAllTransactions: async (req, res) => { /* logic */ },
    getTransaction: async (req, res) => { /* logic */ },
    printReceipt: async (req, res) => { /* logic */ },
    trackStatusPublic
};