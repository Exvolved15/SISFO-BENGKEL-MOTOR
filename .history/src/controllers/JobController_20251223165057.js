...
exports.updateJobStatus = async (req, res) => {
    try {
        ...
        // Sinkronisasi: Jika selesai, bebaskan mekanik dan update header transaksi
        if (status === 'completed') {
            await User.findByIdAndUpdate(job.assignedTo, { status: 'ready' });
        ...
        res.json({ success: true, message: "Status pengerjaan diperbarui" });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};