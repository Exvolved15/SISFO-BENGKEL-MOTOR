...
// Mengubah Status Pekerjaan (Mulai / Selesai)
exports.updateJobStatus = async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body; // status: 'on_progress' atau 'completed'

        const updatedJob = await Job.findByIdAndUpdate(id, { status }, { new: true });

        // Jika pekerjaan SELESAI, kembalikan status mekanik jadi 'tersedia' (ready)
        if (status === 'completed') {
            await User.findByIdAndUpdate(updatedJob.assignedTo, { status: 'ready' });
            // Update juga status di header transaksi
            await Transaction.findByIdAndUpdate(updatedJob.transactionId, { status: 'lunas' });
        } else if (status === 'on_progress') {
            await Transaction.findByIdAndUpdate(updatedJob.transactionId, { status: 'proses' });
        }
