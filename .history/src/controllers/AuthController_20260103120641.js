...
const register = async (req, res) => {
    try {
        ...
        const lowerEmail = email.toLowerCase(); // Standarisasi

        // 1. Hash password untuk keamanan MongoDB
        ...
        // 3. Simpan ke MongoDB Lokal
        const newUser = await User.create({
            uid: userRecord.uid,
            name,
            email: lowerEmail,
            password: hashedPassword, // PENTING: Mencegah error 'Illegal arguments'
            role: role || 'user',
            department: department || '-',
        });

        // Data akan otomatis tersinkron ke Realtime Database via Mongoose Plugin (lihat langkah 3)

        return res.status(201).json({ success: true, message: "User Terdaftar Sinkron" });
    } catch (error) {
