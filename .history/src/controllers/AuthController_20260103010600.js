...
const verifyToken = async (req, res) => {
    ...
    if (!idToken) {
        return res.status(400).json({ success: false, message: "Token Firebase tidak ditemukan" });
    }
    ...
    try {
        ...
        if (!user) {
            ...
            return res.status(404).json({ success: false, message: 'User tidak terdaftar di sistem bengkel.' });
        }
        ...
        // Respon JSON murni untuk Fetch API
        ...
        return res.status(401).json({ success: false, message: 'Token tidak valid atau server error.' });
    }
};
