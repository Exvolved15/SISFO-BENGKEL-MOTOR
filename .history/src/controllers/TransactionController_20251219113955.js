...
const processTransactionLogic = async (reqBody, userCreator) => {
    // 1. Debugging: Lihat apa yang dikirim Frontend
    ...
    try {
        ...
        if (itemDetails && itemDetails.length > 0) {
            ...
            for (const item of items) {
                ...
                let dbItem = null;
                let finalItemType = item.itemType;
                let itemName = "Unknown Item"; // Default value biar gak error validation
                let itemCode = "UNK-001";      // Default value biar gak error validation
                ...
                if (dbItem) {
                    ...
                    // B. Kalau bukan Part, Cari di Service
                    dbItem = await Service.findById(item.itemId);
                    if (dbItem) {
                        // KETEMU DI SERVICE
                        console.log(`✅ [DEBUG] Ditemukan di Service: ${dbItem.serviceName}`);
                        finalItemType = 'service';
                        itemName = dbItem.serviceName;
                        itemCode = dbItem.serviceCode;
                    } else {
                        // C. TIDAK KETEMU DIMANAPUN
                        console.error(`❌ [ERROR] Item ID ${item.itemId} TIDAK ADA di DB Lokal!`);
                        // Kita throw error agar user tahu datanya salah
                        throw new Error(`Data Item (ID: ${item.itemId}) tidak ditemukan di Database Lokal. Silakan Refresh halaman.`);
                    }
                }
