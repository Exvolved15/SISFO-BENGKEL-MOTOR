...
/**
 * @desc    Generate JWT Local Session
 */
...
/**
 * @desc    Logic Register (Firebase + MongoDB)
 */
...
const verifyToken = async (req, res) => {
    ...
    try {
        ...
        // 4. SIMPAN TOKEN KE DATABASE (Untuk validasi single-session/logout)
        user.currentSessionToken = sessionToken;
        await user.save({ validateBeforeSave: false });
        
        console.log(`[AUTH] Sesi dibuat: ${user.email}`); 

        // 5. KIRIM JWT SEBAGAI HTTP-ONLY COOKIE
        // PERBAIKAN: Menggunakan variabel 'sessionToken' (bukan 'token')
...
const logout = async (req, res) => {
    try {
        // Opsional: Hapus token di DB agar tidak bisa digunakan lagi jika dicuri
        if (req.user) {
...
// Placeholder untuk rute lama agar tidak error
