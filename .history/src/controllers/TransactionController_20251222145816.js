...
const printReceipt = async (req, res) => {
    try {
        ...
        // 2. Ambil detail item (Barang/Jasa) agar tabel di resi tidak kosong
        const details = await TransactionDetail.find({ transactionId: req.params.id });
        
        // Log untuk memastikan data diskon terbaca di terminal server
        console.log(`Cetak Resi: ${transaction.invoiceNumber}, Diskon: ${transaction.discount}`);

        // 3. Render ke file receipt.ejs
        res.render('transactions/receipt', { 
            layout: false, // Mematikan layout navbar agar hanya muncul struk
            transaction: transaction, 
            details: details || [] 
        });
    } catch (error) {
        console.error("Error Cetak Resi:", error.message);
