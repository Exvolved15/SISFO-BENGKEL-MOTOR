...
exports.updateJobStatus = async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body; // 'on_progress' atau 'completed'

        const job = await Job.findById(id);
        if (!job) return res.status(404).json({ success: false, message: "Job tidak ditemukan" });

        // 1. Update Status di tabel Job
        job.status = status;
        if (status === 'completed') job.completedAt = Date.now();
        await job.save();

        // 2. SINKRONISASI KE TABEL LAIN
        if (status === 'on_progress') {
            // Update transaksi agar kasir tahu motor sedang dikerjakan
            await Transaction.findByIdAndUpdate(job.transactionId, { status: 'proses' });
        } 
        else if (status === 'completed') {
            // Update transaksi jadi lunas/selesai
            await Transaction.findByIdAndUpdate(job.transactionId, { status: 'lunas' });
            
            // BEBASKAN MEKANIK: Ubah status user dari 'kerja' kembali ke 'ready'
            await User.findByIdAndUpdate(job.mechanicId, { status: 'ready' });
        }
...
// Update Status Pekerjaan oleh Mekanik
