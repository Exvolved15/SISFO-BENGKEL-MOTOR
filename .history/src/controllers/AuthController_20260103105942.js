...
// API REGISTER - Sinkronkan MongoDB & Firebase
const register = async (req, res) => {
    try {
        const { name, email, password, role, department } = req.body;

        // 1. Hash password untuk MongoDB (Mencegah error 'Illegal arguments')
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);

        // 2. Buat User di Firebase Auth untuk Realtime Database
        ...
        try {
            ...
            console.warn("Firebase Sync Warning: " + fError.message);
        }

        // 3. Simpan ke MongoDB (Pastikan field 'password' ada di model User)
...
// --- FITUR LOGIN API (FIXED) ---
const loginAPI = async (req, res) => {
    try {
        const { email, password } = req.body;
        
        // Gunakan .select('+password') karena field password biasanya di-hide di model
        ...
        // Cek apakah password ada di DB lokal
        if (!user.password) {
            return res.status(401).json({ 
                success: false, 
                message: "Akun ini terdaftar via Firebase. Silakan login dengan metode tersebut." 
            });
        }
