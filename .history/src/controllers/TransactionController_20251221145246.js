...
const processTransactionLogic = async (reqBody, userCreator) => {
    ...
    try {
        ...
        // 5. Buat Job HANYA JIKA ada mekanik yang ditugaskan
        if (mechanicId) {
            try {
                await Job.create({
                    jobTitle: notes && notes !== '-' ? notes : `Servis ${vehicleLicense}`, 
                    description: `Plat: ${vehicleLicense}. Items: ${jobDescriptionItems.join(', ')}`,
                    mechanicId: mechanicId, // Ini adalah 'assignedTo' di model Job Anda
                    assignedTo: mechanicId, // Sesuaikan dengan nama field di model Job (biasanya assignedTo)
                ...
                console.error("⚠️ Gagal membuat Job, tapi transaksi tetap disimpan:", jobError.message);
                // Transaksi tidak di-cancel hanya karena job gagal (opsional)
            }
        }
};
