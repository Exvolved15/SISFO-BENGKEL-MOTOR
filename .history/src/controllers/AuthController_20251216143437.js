...
const verifyToken = async (req, res) => {
    ...
    try {
       ...
       // 4. SIMPAN TOKEN BARU KE DATABASE (PASTIKAN sessionToken YANG BENAR)
        // Catatan: Jika sessionToken adalah string JWT, pastikan tidak ada proses hashing
        user.currentSessionToken = sessionToken;
        await user.save({ validateBeforeSave: false }); // <-- FOKUS DI SINI

        // 5. Kirim JWT sebagai HTTP-Only Cookie
        res.cookie('token', sessionToken, { // <-- Token yang dikirim harus SAMA
            httpOnly: true,
