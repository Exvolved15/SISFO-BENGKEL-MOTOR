...
const processTransactionLogic = async (reqBody) => {
    ...
    // --- DEKLARASIKAN VARIABEL UTAMA DI LUAR BLOK TRY DENGAN 'let' ---
    let newTransaction; 
    let detailsToSave = []; // Gunakan let di sini juga
    let partUpdates = [];   // Gunakan let di sini juga
    
    try {
        // 1. GENERATE NOMOR INVOICE
        const invoiceNumber = await generateInvoiceNumber();
        
        // 2. BUAT HEADER TRANSAKSI (Hapus 'const' di depan newTransaction)
        newTransaction = await Transaction.create([{ // <-- Hapus 'const' di sini!
            invoiceNumber,
            customerName,
            vehicleLicense,
            totalAmount,
        ...
        // 6. COMMIT TRANSAKSI
        await session.commitTransaction();
        // Akhiri sesi setelah commit berhasil.
        session.endSession(); // <-- Pindahkan session.endSession() ke sini

        // MENGEMBALIKAN DATA SETELAH BERHASIL
        return { 
            transaction: newTransaction[0], 
            details: savedDetails 
        };
        ...
        // JIKA ERROR, MAKA HANYA DISINI YANG MENJALANKAN ABORT
        
        // Cek apakah sesi masih aktif (belum di commit)
        if (session.inTransaction()) {
             await session.abortTransaction();
        }
        session.endSession();
        
        // Lempar error agar ditangkap oleh ViewRoutes
        throw error; 
    }
};
...
// Di TransactionController.js, pastikan fungsi ini sudah benar
const getAllTransactions = async (req, res) => {
    try {
        // Ambil semua transaksi, urutkan dari yang terbaru (createdAt -1)
        const transactions = await Transaction.find({}).sort({ createdAt: -1 });
        
        // Jika dipanggil dari API (insomnia/postman)
        if (req.originalUrl.startsWith('/api')) {
            return res.status(200).json({ success: true, data: transactions });
        }
        
        // Jika dipanggil dari View (Dashboard)
        return transactions; 
    } catch (error) {
        throw error;
    }
...
const getTransaction = async (req, res) => {
    try {
        const transaction = await Transaction.findById(req.params.id)
            // Anda mungkin ingin melakukan populate di sini
            // .populate('details') 
...
module.exports = {
    ...
    // createTransaction,  <--- HAPUS BARIS INI
    getAllTransactions,
    getTransaction, // <--- PASTIKAN INI DIEKSPOR
