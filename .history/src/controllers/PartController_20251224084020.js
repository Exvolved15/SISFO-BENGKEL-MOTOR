...
const createPartView = async (req, res) => {
    try {
        // 1. Ambil data dari body (pastikan name di EJS sesuai)
        const { name, code, purchasePrice, price, stock, supplier } = req.body;

        // 2. Simpan ke DB dengan field yang sesuai dengan src/models/Part.js
        await Part.create({
            name,
        ...
        // 3. Redirect Sukses dengan parameter untuk SweetAlert
        res.redirect('/parts?success=Data+berhasil+disimpan'); 
    } catch (error) {
        console.error("Error Simpan:", error.message);
        // Redirect Gagal dengan parameter error untuk SweetAlert
