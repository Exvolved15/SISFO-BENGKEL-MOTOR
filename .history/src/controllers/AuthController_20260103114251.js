...
const register = async (req, res) => {
    try {
        ...
        const lowerEmail = email.toLowerCase(); // Standarisasi email

        // 1. Hash password untuk MongoDB
        ...
        // 3. Simpan ke MongoDB dengan UID Firebase
        const newUser = await User.create({
            uid: userRecord.uid,
            name,
            email: lowerEmail,
            password: hashedPassword, // Mencegah error 'Illegal arguments'
            role: role || 'user',
            department: department || '-',
        });

        return res.status(201).json({ success: true, message: "User Terdaftar di MongoDB & Firebase" });
    } catch (error) {
