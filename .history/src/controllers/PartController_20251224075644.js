...
const createPartView = async (req, res, next) => {
    try {
        ...
        // Validasi data di level controller
        if (!purchasePrice) {
        ...
        await Part.create({
            name,
            code,
            purchasePrice: parseFloat(purchasePrice), // Pastikan data ini ditangkap
            price: parseFloat(price),
            stock: parseInt(stock) || 0,
            supplier: supplier || '-'
        }); 

        next(); // Melanjutkan ke res.redirect di rute
    } catch (error) {
        // Kirim error kembali ke form untuk ditampilkan di Pop-up SweetAlert
