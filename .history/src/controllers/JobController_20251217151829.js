...
const createJob = async (req, res) => {
    const { customerName, customerPhone, vehicleModel, licensePlate, assignedTo } = req.body;

    try {
        const assignedMekanik = await User.findById(assignedTo);
        if (!assignedMekanik || assignedMekanik.role !== 'mekanik') {
            return res.status(400).json({ success: false, message: 'Mekanik tidak valid.' });
        }
        ...
        res.status(201).json({ success: true, message: 'Antrean dibuat.', data: newJob });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
module.exports = { getJobs, getMekanikJobs, getMyJobs, createJob, updateJobStatus, addServiceItem };