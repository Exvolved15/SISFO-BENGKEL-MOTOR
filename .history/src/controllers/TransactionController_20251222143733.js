...
const getTransaction = async (req, res) => {
    try {
        ...
        // 2. Ambil data detail (item suku cadang/jasa)
        const details = await TransactionDetail.find({ transactionId: transaction._id });

        // 3. RENDER (Kunci Perbaikan): Kirim sebagai 'header' agar sinkron dengan detail.ejs
        res.render('transactions/detail', { 
            title: 'Detail Transaksi', 
            header: transaction, // JANGAN GANTI NAMA INI
            details: details || [], 
            user: req.user, 
