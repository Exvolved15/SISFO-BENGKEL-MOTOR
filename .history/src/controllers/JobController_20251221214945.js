...
exports.finishJob = async (req, res) => {
    try {
        ...
        // Update status menjadi 'completed' agar pindah ke riwayat
        await Job.findByIdAndUpdate(jobId, {
            status: 'completed',
            completedAt: new Date() // Catat waktu selesai
        });
...
router.post('/api/jobs/finish/:id', async (req, res) => {
    const job = await Job.findByIdAndUpdate(req.params.id, { status: 'completed' });
    
    // SINKRONISASI: Update juga tabel transaksinya agar Kasir & Pelanggan melihat 'Selesai'
    await Transaction.findByIdAndUpdate(job.transactionId, { status: 'completed' });
