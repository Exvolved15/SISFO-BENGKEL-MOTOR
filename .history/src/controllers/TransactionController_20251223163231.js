...
const trackStatusPublic = async (req, res) => {
    try {
        ...
        // Cari berdasarkan Invoice Number ATAU Plat Nomor
...
exports.createTransaction = async (req, res) => {
    try {
        ...
        const newTransaction = await Transaction.create({
            ...
            customer, // Ini harus ID User Pelanggan
            customerName: (await User.findById(customer)).name,
            motorName,
            vehicleLicense,
            totalAmount,
            discount,
            ...
            paymentMethod,
            mechanic: assignedTo || null,
            mechanicName: assignedTo ? (await User.findById(assignedTo)).name : 'Tanpa Mekanik',
            notes,
            status: assignedTo ? 'proses' : 'selesai' // Jika ada mekanik, status "proses"
        });

        // 2. Simpan Detail Item (Suku Cadang & Jasa)
        ...
        // 3. SINKRONISASI KE MEKANIK [PENTING!]
        if (assignedTo) {
            await User.findByIdAndUpdate(assignedTo, { 
                status: 'kerja' // Ubah status mekanik jadi sibuk
            });
        }
