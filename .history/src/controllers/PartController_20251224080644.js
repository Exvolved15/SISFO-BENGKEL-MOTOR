...
const createPartView = async (req, res, next) => {
    try {
        // Ambil data dari req.body (sesuai atribut 'name' di form EJS)
        const { name, code, purchasePrice, price, stock, supplier } = req.body;

        // Validasi manual jika data kosong sebelum kena validasi Mongoose
        if (!purchasePrice || purchasePrice === "") {
            return res.redirect(`/parts/add?error=${encodeURIComponent("Harga Beli tidak terdeteksi oleh sistem!")}`);
        }
        ...
        await Part.create({
            ...
            purchasePrice: parseFloat(purchasePrice), // Harga Beli
            sellingPrice: parseFloat(price),          // Mapping 'price' dari form ke 'sellingPrice' di model
            supplier: supplier || '-'
        });
        ...
        // Redirect balik ke form + kirim pesan error untuk Pop-up SweetAlert
