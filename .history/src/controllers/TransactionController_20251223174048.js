...
exports.createTransaction = async (req, res) => {
    try {
        ...
        // 5. SINKRONISASI KE TABEL JOB (Agar muncul di Dashboard Mekanik & Pelanggan)
        if (assignedTo && assignedTo !== "null") {
            await Job.create({
                invoiceNumber: invoiceNumber,
                transactionId: newTransaction._id,
                customerId: customer || null, // Link untuk Dashboard Pelanggan
                customer: { name: customerData ? customerData.name : 'Umum' },
                vehicleLicense: vehicleLicense,
                motorName: motorName || '-',
                mechanicId: assignedTo, // Link untuk Dashboard Mekanik
                description: "Pengerjaan servis rutin dan penggantian part sesuai invoice.",
                status: 'pending'
            });
