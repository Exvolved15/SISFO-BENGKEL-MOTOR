...
const createPartView = async (req, res) => {
    try {
        ...
        if (!purchasePrice || purchasePrice <= 0) {
            return res.redirect(`/parts/add?error=${encodeURIComponent("Harga beli wajib diisi dan harus angka positif!")}`);
        }
        ...
        await Part.create({
            ...
            sellingPrice: parseFloat(price), // Mapping 'price' ke 'sellingPrice'
            supplier: supplier || '-'
        });

        // REDIRECT SUKSES: Kembali ke halaman daftar atau tetap di halaman tambah
        ...
        // Cek jika error karena data duplikat (Kode/Nama sama)
        if (error.code === 11000) {
            pesanError = "Kode atau Nama Suku Cadang sudah ada di sistem!";
        } else if (error.name === 'ValidationError') {
            pesanError = "Data tidak valid: " + Object.values(error.errors).map(el => el.message).join(', ');
        }

        // REDIRECT ERROR: Kembali ke form dengan pesan error
