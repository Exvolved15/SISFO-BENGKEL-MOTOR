...
const getTransaction = async (req, res) => {
    try {
        ...
        // Ambil data item (suku cadang/jasa) untuk tabel di dalam detail
        const details = await TransactionDetail.find({ transactionId: transaction._id });
        
        // Render file detail.ejs dengan variabel 'header'
        res.render('transactions/detail', { 
            title: 'Detail Transaksi', 
            header: transaction, // Nama variabel ini harus sama dengan di detail.ejs
            details: details, 
            user: req.user, 
...
const printReceipt = async (req, res) => {
    try {
        ...
        // WAJIB: Cari detail agar 'details' tidak undefined di EJS
        const details = await TransactionDetail.find({ transactionId: req.params.id });
        
        console.log(`Cetak Resi: ${transaction.invoiceNumber}, Items: ${details.length}`);
