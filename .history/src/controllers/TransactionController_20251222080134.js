...
const processTransactionLogic = async (reqBody, userCreator) => {
    ...
    // 2. Cari data User berdasarkan ID untuk mendapatkan nama aslinya
    let finalCustomerName = "Umum";
    if (customer) {
        const foundCustomer = await User.findById(customer);
        if (foundCustomer) {
            finalCustomerName = foundCustomer.name; // Inilah nama/nickname yang Anda inginkan
        }
    }
    ...
    try {
        ...
        // 3. Simpan Transaksi dengan ID Pelanggan agar muncul di dashboard mereka
        const newTransaction = await Transaction.create({
            invoiceNumber,
            customer: customer,      // Simpan ID (untuk dashboard pelanggan)
            customerName: finalCustomerName, // Simpan Nama (untuk laporan kasir)
            vehicleLicense,
            // ... sisa field lainnya tetap sama
            status: 'proses', // Gunakan 'proses' agar mekanik bisa mengambil
    ...
    customerId: reqBody.customer, // WAJIB: Masuk ke sub-document customer.name
...
exports.createTransaction = async (req, res) => {
    try {
        // Cek apakah ada item kategori 'jasa' di dalam detail transaksi
        const hasService = req.body.items.some(item => item.category === 'jasa');

        const newTransaction = await Transaction.create({
            ...req.body,
            // Jika ada jasa, status 'on_progress'. Jika hanya barang, status 'completed'
            status: hasService ? 'on_progress' : 'completed'
        });
