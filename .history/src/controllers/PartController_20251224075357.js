...
const createPartView = async (req, res, next) => {
    try {
        ...
        if (!name || !code || !purchasePrice || !price) {
            throw new Error("Nama, Kode, Harga Beli, dan Harga Jual wajib diisi!");
        }
        ...
        next(); // Lanjut ke redirect di viewRoutes
    } catch (error) {
        // Kirim error ke URL agar ditangkap SweetAlert di EJS
