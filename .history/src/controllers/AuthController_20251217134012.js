...
const verifyToken = async (req, res) => {
    ...
    try {
        // 1. Verifikasi ID Token Firebase NYATA
        const decodedToken = await admin.auth().verifyIdToken(idToken); 
        const firebaseUid = decodedToken.uid; 
        
        // 2. Cari User di MongoDB (Deklarasi 'user' HANYA SATU KALI)
        ...
        // 4. SIMPAN TOKEN BARU KE DATABASE (Hanya sekali!)
        user.currentSessionToken = sessionToken;
        await user.save({ validateBeforeSave: false });
        
        console.log(`[AUTH CONTROLLER] Token sesi baru berhasil disimpan untuk user: ${user.email}`); 

        // 5. KIRIM JWT BARU SEBAGAI HTTP-ONLY COOKIE
        res.cookie('token', token, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            sameSite: 'lax', // Membantu masalah redirect di localhost
