...
// Fungsi untuk Dashboard Admin (Profit Analysis)
exports.getProfitReport = async (req, res) => {
    try {
        const report = await Transaction.aggregate([
            ...
            { $lookup: {
                from: 'parts',
                localField: 'details.partId',
                foreignField: '_id',
                as: 'partInfo'
            }},
            { $project: {
                totalSales: { $multiply: ['$details.price', '$details.quantity'] },
                totalCost: { $multiply: [{ $arrayElemAt: ['$partInfo.purchasePrice', 0] }, '$details.quantity'] },
                date: '$createdAt'
            }},
            { $group: {
                _id: { $dateToString: { format: "%Y-%m-%d", date: "$date" } },
                dailyProfit: { $sum: { $subtract: ["$totalSales", "$totalCost"] } },
                dailyRevenue: { $sum: "$totalSales" }
            }},
            { $sort: { _id: -1 } }
        ]);
        res.render('admin/report', { title: 'Laporan Keuangan', report, user: req.user });
