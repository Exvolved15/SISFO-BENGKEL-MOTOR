...
const register = async (req, res) => {
    try {
        ...
        // 1. Hash password agar field password di MongoDB TIDAK UNDEFINED
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);

        // 2. Sinkronisasi ke Firebase Auth (Opsional)
        let firebaseUid = null;
        try {
            const userRecord = await admin.auth().createUser({ email, password, displayName: name });
            firebaseUid = userRecord.uid;
        } catch (fError) { console.warn("Firebase Register Skip: " + fError.message); }

        // 3. Simpan ke MongoDB (Sertakan hashedPassword)
        const newUser = await User.create({
            uid: firebaseUid, 
            name, email,
            password: hashedPassword, // PENTING: Untuk mencegah error 401/Illegal arguments
            role: role || 'user',
