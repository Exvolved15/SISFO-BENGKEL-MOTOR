...
const createJob = async (req, res) => {
    // Data yang diharapkan dari Kasir/Form
    const { 
        customerName, 
        customerPhone, 
        vehicleModel, 
        licensePlate, 
        serviceDetails, 
        assignedToUid // UID Mekanik yang ditugaskan (dari Firebase/MongoDB)
    } = req.body;

    // Tambahan: Tambahkan Mekanik yang Ditugaskan ke database MongoDB
    try {
        // Cari ID MongoDB Mekanik berdasarkan UID Firebase-nya (Asumsi assignedToUid adalah UID)
        // Jika Anda menggunakan Mekanik ID (_id) langsung dari frontend, ganti logika ini.
        const assignedMekanik = await User.findOne({ uid: assignedToUid, role: 'mekanik' });
        ...
        const newJob = await Job.create({
            customer: { // Asumsi model Job memiliki sub-dokumen atau field customer
                name: customerName,
                phone: customerPhone,
            },
            vehicle: {
            ...
            assignedTo: assignedMekanik._id, // Simpan ID MongoDB Mekanik
            status: 'pending', // Status awal
            createdBy: req.user._id, // ID Kasir yang membuat Job
        });

        // 2. Respons Sukses
        res.status(201).json({ 
            success: true, 
            message: 'Pekerjaan baru berhasil dibuat dan ditugaskan.',
            data: newJob
        });
