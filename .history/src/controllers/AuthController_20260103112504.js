...
// API REGISTER SINKRON
const register = async (req, res) => {
    try {
        const { name, email, password, role, department } = req.body;

        // 1. Hash password untuk simpan di MongoDB Lokal
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);

        // 2. Buat User di Firebase Auth
        const userRecord = await admin.auth().createUser({
            email: email.toLowerCase(), // Pastikan lowercase untuk konsistensi
            password,
            displayName: name,
        });

        // 3. Simpan ke MongoDB dengan UID dari Firebase
        const newUser = await User.create({
            uid: userRecord.uid,
            name,
            email: email.toLowerCase(),
            password: hashedPassword, // Disimpan agar bcrypt tidak error
            role: role || 'user',
            department: department || '-',
        });

        return res.status(201).json({ success: true, message: "Registrasi Berhasil & Sinkron" });
    } catch (error) {
