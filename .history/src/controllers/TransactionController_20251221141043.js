...
const processTransactionLogic = async (reqBody, userCreator) => {
    ...
    if (itemDetails && itemDetails.length > 0) {
        ...
        for (const item of items) {
            ...
            // 1. Cari data di koleksi Part jika tipenya 'part'
            if (item.itemType === 'part') {
                dbItem = await Part.findById(item.itemId);
                if (dbItem) {
                    itemName = dbItem.name; // Mendapatkan nama dari model Part
                    itemCode = dbItem.code; // Mendapatkan kode dari model Part
            ...
            // 2. Cari data di koleksi Service jika tipenya 'service'
            else if (item.itemType === 'service') {
                dbItem = await Service.findById(item.itemId);
                if (dbItem) {
                    // FIX: Gunakan .name dan .code, BUKAN .serviceName
                    itemName = dbItem.name; 
                    itemCode = dbItem.code; 
                }
            }
        
            // 3. Jika item tidak ditemukan di database manapun
            if (!dbItem) {
                throw new Error(`Item dengan ID ${item.itemId} tidak ditemukan di database.`);
            }
        
            // 4. Masukkan ke array detail dengan field yang sudah terisi
            detailsToSave.push({
                transactionId,
                itemType: item.itemType,
                itemId: item.itemId,
                itemName: itemName, // Sekarang dipastikan tidak 'undefined'
                itemCode: itemCode, // Sekarang dipastikan tidak 'undefined'
