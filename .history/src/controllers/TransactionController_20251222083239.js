...
const processTransactionLogic = async (reqBody, userCreator) => {
    ...
    // 1. Cari Nama Pelanggan (Nickname) berdasarkan ID
    ...
    try {
        ...
        const newTransaction = await Transaction.create({
            invoiceNumber,
            customer: customer || null,      // Menyimpan ID untuk Dashboard Pelanggan
            customerName: finalCustomerName, // Menyimpan Nama untuk Tabel Riwayat
            vehicleLicense,
            mechanicId,
            mechanicName,
        ...
        if (mechanicId) {
            await Job.create({
                invoiceNumber,
                customerId: customer || null, // Link agar pelanggan bisa cek progres servis
                transactionId,
                vehicleLicense,
