...
const processTransactionLogic = async (reqBody, userCreator) => {
    ...
    try {
        ...
        // --- BAGIAN BARU: SINKRONISASI KE JOB MEKANIK ---
        if (mechanicId) {
            try {
                await Job.create({
                    jobTitle: notes && notes !== '-' ? notes : `Servis ${vehicleLicense}`, 
                    description: `Daftar Pekerjaan: ${jobDescriptionItems.join(', ')}`,
                    mechanicId: mechanicId, // ID Mekanik dari form
                    transactionId: transactionId, // Hubungkan ke transaksi ini
                    vehicleLicense: vehicleLicense,
                    customerName: customerName,
                    status: 'pending' // Status awal
                });
                console.log(`✅ [JOB] Berhasil membuat antrean kerja untuk: ${mechanicName}`);
            } catch (jobError) {
                console.error("⚠️ Gagal membuat Job:", jobError.message);
                // Kita tidak men-cancel transaksi hanya karena Job gagal, tapi beri log.
            }
        }
