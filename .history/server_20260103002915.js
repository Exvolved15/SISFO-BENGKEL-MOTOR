...
app.use(
    helmet({
        contentSecurityPolicy:false,// {
            directives: {
                ...
                "connect-src": [
                    ...
                    "http://localhost:5000", 
                    "http://127.0.0.1:5000", 
                    "https://cdn.jsdelivr.net", // Izinkan CDN untuk sourcemaps
        ...
        // WAJIB: Set hsts ke false agar localhost tidak dipaksa ke HTTPS
        hsts: false 
    })
);
...
// A. Rute Auth & API Publik
app.use('/api/auth', authRoutes);

// B. Rute API Terproteksi (Kembalikan JSON jika error)
...
// C. Rute Tampilan (HTML)
app.use('/', authViewRoutes); 
app.get('/about', (req, res) => res.render('about', { title: 'Tentang Kami', activePage: 'about' }));
app.use('/', viewRoutes); // Home/Overview

// D. Error Handling untuk API (PENTING)
// Tambahkan ini agar jika rute /api/ tidak ditemukan, dia kirim JSON, bukan HTML 404
app.use('/api/*', (req, res) => {
    res.status(404).json({ message: 'Endpoint API tidak ditemukan' });
});

// E. Error Handling untuk Halaman (HTML)
app.use((req, res) => {
