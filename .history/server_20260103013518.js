...
// PENTING: Gunakan prefix unik atau pastikan urutan agar tidak bentrok
// --- C. Tampilan Terproteksi ---
app.use('/admin/dashboard', protect, async (req, res) => {
    try {
        // Ambil semua data yang dibutuhkan dashboard secara paralel
        ...
        res.render('admin/dashboard', { 
            ...
            users: users, 
            parts: parts, // Variabel ini yang ditanyakan oleh error baris 78
            activePage: 'dashboard'
        });
    } catch (error) {
        console.error("Dashboard Error:", error);
        res.status(500).send("Gagal memuat data statistik dashboard");
    }
});

app.use('/mechanic/dashboard', protect, (req, res) => res.render('mechanic/dashboard', { title: 'Mekanik' }));
app.use('/customer/dashboard', protect, async (req, res) => {
    try {
        // AMBIL DATA PEKERJAAN (JOBS) MILIK CUSTOMER YANG LOGIN
        // Kita mencari Job yang kolom 'customer' (atau 'user') nya sama dengan req.user._id
        const myJobs = await Job.find({ customer: req.user._id })
                                .sort({ createdAt: -1 }); // Urutkan dari yang terbaru
        
        // KIRIM DATA myJobs KE EJS
        res.render('customer/dashboard', { 
            title: 'My Dashboard', 
            myJobs: myJobs, // Variabel yang dibutuhkan EJS di baris 43
            activePage: 'dashboard',
            user: req.user
        });
    } catch (error) {
        console.error("Customer Dashboard Error:", error);
        res.status(500).send("Gagal memuat data dashboard customer");
    }
});
...
app.get('/about', (req, res) => res.render('about', { title: 'Tentang Kami' }));

// Rute Beranda
app.get('/', (req, res) => {
    if (req.cookies.token) {
        // Decode token untuk tahu role dan redirect otomatis
        try {
